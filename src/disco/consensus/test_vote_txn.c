...
ushort
dummy_vote_instr_fun( uchar * out_buf ) {
...
main( int argc, char ** argv ) {
  ...
  ulong base_size = fd_txn_base_generate( txn_meta_buf, txn_buf, 2, &vote_txn_accounts, NULL );
  FD_LOG_NOTICE(("fd_txn_base_generate returns %lu", base_size));
  ...
  uchar vote_instr[FD_TXN_MTU];
  ushort vote_instr_raw_size = dummy_vote_instr_fun(vote_instr);
  ulong size = fd_txn_add_instr(txn_meta_buf, txn_buf, 2, instr_accounts, 2, vote_instr, vote_instr_raw_size);
  FD_LOG_NOTICE(("Create a vote transaction: txn raw_size=%u; instr raw_size=%u.", size, vote_instr_raw_size));

  /* Add signatures */
  //txn_buf[0] = (uchar)vote_txn_accounts.signature_cnt;
  /* TODO: sign the transaction message and put the sigatures in txn_buf  */
