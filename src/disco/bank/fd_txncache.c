...
fd_txncache_ensure_txnpage( fd_txncache_t *                    tc,
  ...
  if( FD_LIKELY( FD_ATOMIC_CAS( &blockcache->pages[ page_cnt ], UINT_MAX, UINT_MAX-1UL )==UINT_MAX ) ) {
    ...
    for(;;) {
      ...
      ulong old_txnpages_free_cnt = FD_ATOMIC_CAS( &tc->txnpages_free_cnt, txnpages_free_cnt, txnpages_free_cnt-1UL );
...
fd_txncache_insert_txn( fd_txncache_t *                        tc,
  ...
  for(;;) {
    ...
    for(;;) {
      ...
      FD_COMPILER_MFENCE();
      if( FD_LIKELY( FD_ATOMIC_CAS( &blockcache->heads[ txn_bucket ], head, FD_TXNCACHE_TXNS_PER_PAGE*txnpage_idx+txn_idx )==head ) ) break;
      FD_SPIN_PAUSE();
    }
    ...
    for(;;) {
      ...
      FD_COMPILER_MFENCE();
      if( FD_LIKELY( FD_ATOMIC_CAS( &slotblockcache->heads[ txn_bucket ], head, FD_TXNCACHE_TXNS_PER_PAGE*txnpage_idx+txn_idx )==head ) ) break;
      FD_SPIN_PAUSE();
    }
